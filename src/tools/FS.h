...
public:
	...
	{	Pos=ptr;	assert((Pos<=Size) && (Pos>=0));	};
	IC void	Advance(int cnt)		// перемещение указателя
	{	Pos+=cnt;	assert((Pos<=Size) && (Pos>=0));	};
...
public:
	CVirtualFileStreamRW(const char *cFileName) {
		...
		if (hSrcFile == INVALID_HANDLE_VALUE)
			hSrcFile = CreateFile(cFileName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_NEW, 0, 0);
