...
namespace engine
{
	void strncpyz(char* dest, const char* src, int destsize) {
		if (!src) {
			Core::error("strncpyz: NULL src");
		}
		if (destsize < 1) {
			Core::error("strncpyz: destsize < 1");
		}
...
#define	MAX_PARSE_INFO	16
	...
	*/
	void Com_BeginParseSession(const char* filename) {
		if (parseInfoNum == MAX_PARSE_INFO - 1) {
			Core::error("Com_BeginParseSession: session overflow");
		}
		parseInfoNum++;
		pi = &parseInfo[parseInfoNum];

		pi->lines = 1;
		strncpyz(pi->parseFile, filename, sizeof(pi->parseFile));
	}
	...
	*/
	void Com_EndParseSession(void) {
		if (parseInfoNum == 0) {
			Core::error("Com_EndParseSession: session underflow");
		}
		parseInfoNum--;
		pi = &parseInfo[parseInfoNum];
	}
	...
	*/
	void Com_ScriptError(const char* msg, ...) {
		...
		Core::error("File %s, line %i: %s", pi->parseFile, pi->lines, string);
	}
	...
	void Com_ScriptWarning(const char* msg, ...) {
		...
		Core::msg("WARNING! File %s, line %i: %s", pi->parseFile, pi->lines, string);
	}
	...
	*/
	static char* Com_ParseExt(const char* (*data_p), bool allowLineBreaks) {
		...
		if (!data_p) {
			Core::error("Com_ParseExt: NULL data_p");
		}
